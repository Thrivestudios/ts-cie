...
function LastFM(options){
	...
	var internalCall = function(params, callbacks, requestMethod){
		...
		/* Cross-domain GET request (JSONP). */
			...
			/* Set script source. */
			script.src = 'data/proxy.php?max_cache_age=604800000&content_type=text/javascript&callback=' + jsonp + '&url='
				+ encodeURIComponent(apiUrl + '?' + array.join('&').replace(/%20/g, '+'));
